...
export class DataLoader {
    ...
    async initData() {
        try {
            // 先获取所有可用时间戳
            const timestampsResponse = await fetch('http://127.0.0.1:5000/api/timestamps');
    ...
    async preloadDataByTimestamp(timestamp) {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/api/contacts/${timestamp}`);
    ...
    async loadDataByTimestamp(timestamp) {
        ...
        if (this.dataCache.has(timestamp)) {
            ...
            console.log(`从缓存加载时间戳 ${timestamp}：${this.contacts.length} 条记录`);
        ...
        try {
            ...
            console.log(`加载时间戳 ${timestamp}：${this.contacts.length} 条记录`);
            return true;
        } catch (error) {
    ...
    async queryUserContacts(userId) {
        try {
            const response = await fetch(`http://127.0.0.1:5000/api/user/${userId}/contacts`);
            if (!response.ok) {
    ...
    async queryUserSecondaryContacts(userId) {
        try {
            const response = await fetch(`http://127.0.0.1:5000/api/user/${userId}/secondary-contacts`);
            if (!response.ok) {
    ...
    async getContactTrajectory(id1, id2) {
        try {
            ...
            const response = await fetch(`http://127.0.0.1:5000/api/trajectory/${a}/${b}`);
            if (!response.ok) {
    ...
    async getBoundsInfo() {
        try {
            const response = await fetch('http://127.0.0.1:5000/api/bounds');
            if (!response.ok) {
