...
class ParquetDataLoader:
    ...
    def __init__(self, base_path: str):
        ...
        Args:
            base_path: Parquet 数据的基础路径（包含 contacts 和 contacts2 目录）
        """
        self.base_path = Path(base_path)
        self.contacts_path = self.base_path / 'contacts'
        self.contacts2_path = self.base_path / 'contacts2'
        ...
        self._max_cache_size = 200  # 最多缓存200个时间戳的数据
        
        # 缓存Parquet文件的元数据（schema和statistics）
        self._parquet_metadata_cache = {}
        ...
        self._user_cache_size = 500  # 最多缓存500个用户查询
        
        # 缓存轨迹查询结果
        self._trajectory_cache = OrderedDict()  # {(id1, id2): trajectory_list}
        self._trajectory_cache_size = 300  # 最多缓存300个轨迹查询
    ...
    def _get_timestamps_from_data(self, table_name: str) -> set:
        ...
        try:
            ...
            print(f"Warning: 扫描 {table_name} 时间戳失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def get_contacts_by_timestamp(self, timestamp: int) -> List[Dict]:
        ...
        if self.contacts_path.exists():
            try:
                ...
                print(f"Warning: 读取 contacts 数据失败: {e}")
        ...
        if self.contacts2_path.exists():
            try:
                ...
                print(f"Warning: 读取 contacts2 数据失败: {e}")
    ...
    def get_bounds(self) -> Dict[str, float]:
        ...
        if self.contacts_path.exists():
            try:
                ...
                print(f"Warning: 读取 contacts 边界失败: {e}")
        ...
        if self.contacts2_path.exists():
            try:
                ...
                print(f"Warning: 读取 contacts2 边界失败: {e}")
    ...
    def get_trajectory(self, id1: int, id2: int) -> List[Dict]:
        ...
        # 查询 contacts 中的轨迹（使用pandas向量化过滤）
        if self.contacts_path.exists():
            try:
                table = pq.read_table(
                    self.contacts_path,
